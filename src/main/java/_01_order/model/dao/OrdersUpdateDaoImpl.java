...
public class OrdersUpdateDaoImpl implements OrdersUpdateDao {
	...
	public int updateOrderConfirmUser(ORDERS order) {
		String updateOrderConfirmUserSql = "UPDATE ORDERS SET" + " order_confirm_user = "
				+ order.getOrder_confirm_user() + " ,order_confirm_time = " + order.getOrder_confirm_time()
				+ " ,order_status = " + order.getOrder_status() + " ,order_finished_time = "
				+ order.getOrder_finished_time() + " WHERE order_id = " + order.getOrder_id();
		int count = 0;
		try (Connection conn = ds.getConnection();
				PreparedStatement ps = conn.prepareStatement(updateOrderConfirmUserSql);) {
